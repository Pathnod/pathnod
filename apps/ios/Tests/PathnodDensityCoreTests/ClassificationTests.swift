...
struct ServiceUUIDTests {
  ...
  @Test(
    "rejects malformed spellings instead of guessing",
    arguments: [
      "",
      "18",
      "180",
...
struct AdvertisementClassifierTests {
  ...
  func localNameFlagChangesNothing() throws {
    ...
    let withoutName = classifier.classify(
      advertisement(services: services, carriesLocalName: false))

    #expect(withName == withoutName)
  }
}

extension ClassificationOutcome {
  /// A matched outcome always names the rule that produced it.
  fileprivate var sourceIsDocumented: Bool {

...
struct DensityExportTests {
  ...
  func fileNameIsIdentifierFree() throws {
    ...
    #expect(
      DensityExport.fileName(for: document) == "pathnod-density-\(fixedSessionID().uuidString).json"
    )
    for identifier in session.peripherals {
  ...
  func emptySessionExports() throws {
    ...
    let accumulator = SessionAccumulator(
      classifier: AdvertisementClassifier(
        registry: try ClassificationRegistry.empty(version: "empty-1")),
      clock: clock,
      makeSessionID: { fixedSessionID() }
    )
    accumulator.start()
  ...
  @Test(
    "an unfinished session cannot be exported",
    arguments: [
      SessionState.idle,

...
struct DensityScanView: View {
            ...
            if controller.summary.byRule.isEmpty {
                ...
                ForEach(controller.summary.byRule, id: \.ruleID) { tally in
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(label(for: tally.category)) · \(tally.confidence.rawValue) confidence · \(tally.sourceKind.rawValue)")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
    ...
    private var exportCard: some View {
        card {
            Text("Export")
                .font(.headline)
            Text("One JSON file with aggregate counters, the ruleset version and the limitations of the method. No device appears in it.")
                .font(.callout)
                .foregroundStyle(.secondary)
    ...
    private var provenanceCard: some View {
        card {
            ...
            Text("Counting an advertiser is not proof of network membership, ownership, online status or location.")
                .font(.footnote)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private var stateDetail: String {
        ...
        case .interrupted:
            return "Scanning stopped. Nothing was counted while the app was not in the foreground. Tap Resume to continue."
        case .finished:
